...
def download_speed_test(alive, proxy, apiurl, timeout, download_test_url):
    try:
        r = requests.get(url=apiurl + '/proxies/' + str(proxy['name']) + '/delay?url='+download_test_url+'&timeout=' + str(timeout), timeout=10)
